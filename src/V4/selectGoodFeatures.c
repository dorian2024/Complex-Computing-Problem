  ...
  selectionMode mode)
{
     ...
     of the two eigenvalues of the Z matrix */
  {
      ...
      int y_points = ((nrows - 2 * bordery) / (tc->nSkippedPixels + 1));
      int x_points = ((ncols - 2 * borderx) / (tc->nSkippedPixels + 1));
      int total_points = y_points * x_points;
...
//#pragma acc parallel loop gang vector collapse(2) \
            copyin(gradx_data[0:ncols*nrows], grady_data[0:ncols*nrows]) \
            private(gxx, gxy, gyy, val, xx, yy)
      for (int iy = 0; iy < y_points; iy++) {
          for (int ix = 0; ix < x_points; ix++) {

              y = bordery + iy * (tc->nSkippedPixels + 1);
              x = borderx + ix * (tc->nSkippedPixels + 1);
...
//#pragma acc loop seq collapse(2)
              ...
              int point_idx = iy * x_points + ix;
              val = (gxx + gyy - sqrtf((gxx - gyy) * (gxx - gyy) + 4 * gxy * gxy)) / 2.0f;
              if (val > limit) val = (float)limit;

              pointlist[point_idx * 3 + 0] = x;
              pointlist[point_idx * 3 + 1] = y;
              pointlist[point_idx * 3 + 2] = (int)val;
          }
      }
